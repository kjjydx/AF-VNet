...
def get_net(config):
    name = config.net
    assert name in ['unet', 'vnet', 'vnet_att'], "Network Name not valid or not supported! Use one of ['unet', 'vnet']"
    if name == 'vnet':
    ...
    elif name=="lightmunet":
        return LightMUNet(out_channels=config.num_outs, in_channels=1, init_filters=32)
    elif name=="vnet_att":
        return VNet3D_att(num_outs=config.num_outs, channels=config.num_channels)
...
def run(config):
    ...
    #load model
    state_dict = torch.load("logs/vnet_att/focal_loss/model_epoch_0095.pht", map_location="cpu")
    #del state_dict["out_conv.conv_1x1.weight"]
    #del state_dict["out_conv.conv_1x1.bias"]
    #del state_dict["out_conv.bn_1x1.weight"]
    #del state_dict["out_conv.bn_1x1.bias"]
    #del state_dict["out_conv.bn_1x1.running_mean"]
    #del state_dict["out_conv.bn_1x1.running_var"]
    ...
    net = train_model(net, optimizer, train_data_loader_3D, val_data_loader_3D,
                      config, device=cuda_dev, logs_folder=logs_folder)

    torch.save(net.state_dict(),os.path.join(logs_folder,"model.pt"))
